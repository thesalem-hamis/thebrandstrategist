...
const EMPTY: Omit<Project, "created_at" | "updated_at"> & { id: string } = {
  ...
  banner_image: null, logo_image: null, link: null, sort_order: 0, active: true,
};
...
export default function DashboardProjects() {
  ...
  const [uploadLogoMsg, setUploadLogoMsg] = useState<{ tone: "ok" | "err"; text: string } | null>(null);
  ...
  function openNew() {
    ...
    setSaveError(null); setUploadMsg(null); setUploadLogoMsg(null); setShowEditor(true);
  }
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!["image/png","image/jpeg","image/jpg","image/webp","image/svg+xml"].includes(file.type)) {
      setUploadLogoMsg({ tone: "err", text: "PNG, JPG, WebP or SVG only." }); return;
    }
    if (file.size > 4 * 1024 * 1024) {
      setUploadLogoMsg({ tone: "err", text: "Max 4 MB." }); return;
    }
    setUploadingLogo(true); setUploadLogoMsg(null);
    try {
      const ext = file.name.split(".").pop()?.toLowerCase() ?? "png";
      const path = `project-logos/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
      ...
      setForm(f => ({ ...f, logo_image: urlData.publicUrl }));
      setUploadLogoMsg({ tone: "ok", text: "Logo uploaded!" });
    } catch (err) {
      setUploadLogoMsg({ tone: "err", text: err instanceof Error ? err.message : "Upload failed." });
    } finally {
            ...
            <form onSubmit={save} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <Field label="Client Logo">
                  <input ref={logoRef} type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={handleLogoUpload} className="hidden" />
                  {form.logo_image ? (
                    <div className="relative group">
                      <div className="flex h-40 w-full items-center justify-center rounded-xl border border-neutral-200 bg-neutral-50">
                        <img src={form.logo_image} alt="Logo" className="max-h-24 max-w-[70%] object-contain" />
                      </div>
                      <button type="button" onClick={() => setForm(f => ({ ...f, logo_image: null }))} className="absolute top-2 right-2 grid h-8 w-8 place-items-center rounded-full bg-black/50 text-white opacity-0 group-hover:opacity-100 hover:bg-black/70">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  ) : (
                    <button type="button" onClick={() => logoRef.current?.click()} disabled={uploadingLogo} className="flex h-40 w-full flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-neutral-200 bg-neutral-50/40 transition-all hover:border-[#5D1F17]/40 hover:bg-[#5D1F17]/5 disabled:opacity-50">
                      {uploadingLogo ? <><Loader2 className="h-5 w-5 animate-spin text-[#5D1F17]" /><span className="text-[11px] text-neutral-500">Uploading…</span></> : <><Upload className="h-5 w-5 text-neutral-400" /><span className="text-[11px] text-neutral-500">Click to upload logo</span><span className="text-[10px] text-neutral-400">PNG, JPG, WebP, SVG up to 4MB</span></>}
                    </button>
                  )}
                  {uploadLogoMsg && (
                    <div className={`mt-2 flex items-center justify-between gap-2 rounded-xl border px-4 py-2.5 text-xs font-medium ${uploadLogoMsg.tone === "ok" ? "border-emerald-200/60 bg-emerald-50 text-emerald-700" : "border-red-200/60 bg-red-50 text-red-700"}`}>
                      <span className="flex items-center gap-2">
                        {uploadLogoMsg.tone === "ok" ? <CheckCircle2 className="h-3.5 w-3.5" /> : <AlertCircle className="h-3.5 w-3.5" />}
                        {uploadLogoMsg.text}
                      </span>
                      {uploadLogoMsg.tone === "err" && <button type="button" onClick={() => logoRef.current?.click()} className="inline-flex items-center gap-1 rounded-full bg-white px-2 py-1 text-[10px] font-bold uppercase text-red-700 hover:bg-red-100"><RotateCcw className="h-2.5 w-2.5" />Retry</button>}
                    </div>
                  )}
                </Field>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" disabled={saving || uploading || uploadingLogo} className="group inline-flex items-center gap-2 rounded-full bg-[#5D1F17] px-6 py-2.5 text-xs font-semibold text-white transition-all hover:bg-[#4A1812] hover:shadow-lg hover:shadow-[#5D1F17]/20 disabled:opacity-50">
